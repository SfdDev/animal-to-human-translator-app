...
import {
  ...
  paginateArticles,
  parseArticlePage,
} from "../content/article-pagination";
...
const categories = computed(() => articlesByCategory(articles.value));
...
watch(
  [page, totalPages, activeId, () => route.name, loading],
  ([currentPage, pages, category, routeName, isLoading]) => {
    if (isLoading || pages < 1) return;
...
<template>
  <ContentPage>
    ...
    <p v-if="loading" class="muted">Загружаем статьи…</p>
